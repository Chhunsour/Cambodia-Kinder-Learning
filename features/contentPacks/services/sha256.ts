 ...
 * Works seamlessly in React Native, Expo, Web, and Node.js environments.
 */

// Pure JS SHA-256 implementation fallback for zero-dependency portability
...
// Safe dynamic require for Node environment that avoids Metro bundler static analysis
...
export async function computeSha256(content: string): Promise<string> {
  // 1. Try Node.js crypto if in Node environment (e.g. test runner)
  try {
    const nodeCrypto = getNodeModule("crypto");
  ...
  // 3. Fallback to pure JS SHA-256
  return sha256Pure(content);
}

/**
 * Synchronous version using pure JS / node crypto.
 */
